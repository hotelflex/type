...
const isCustomType = type => getTypeName(type) === 'CustomType'
...
class Resolver {
  resolve(type, data) {
    if (_.isNil(data)) {
      ...
      return this.resolveCustomType(type, data)
    } else {
  ...
  resolveBaseType(type, data) {
    ...
    if (typeName === 'String') {
      ...
      return isArray(data) ? data.map(d => this.resolve(type[0], d)) : throwTypeError(type, data)
    } else {
      throw new Error('Unknown type name -', typeName)
    }
  }

  resolveCustomType(type, data) {
    const resolved = {}
    Object.keys(type.definition).forEach((name) => {
      const { type: _type, required: _required, default: _default, validator: _validator } = type.definition[name]
      const val = data[name]
      let resolvedVal

      if (_.isNil(val)) {
        if (!_.isNil(_default)) {
          resolvedVal = this.resolve(_type, _default) // TODO: put this in the Parser so its caught at setup time
        } else {
          if (_required) throwValidationError(type, data)
        }
      } else {
        resolvedVal = this.resolve(_type, val)
        if (_validator && !_validator(resolvedVal)) throwValidationError(type, data)
      }
    ...
    if (type.validator && !type.validator(data)) throwValidationError(type, data)
